...
router.post('/', (req, res) => {
    ...
    db.query(
        ...
        (student_id, course_id, assessment_name, category, due_date, earned_marks, total_marks, status) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [
            studentId,
            course_id,
...
router.get('/:courseId/average', (req, res) => {
    ...
    db.query(
        `SELECT 
            ...
            ROUND((SUM(earned_marks) / SUM(total_marks)) * 100, 2) as average
         FROM student_grades 
    ...
    db.query(
        ...
        (err, results) => {
            ...
            const overallGpa = gpaResults.length > 0
                ? (
                    gpaResults.reduce((sum, r) => sum + r.gpa, 0) / gpaResults.length
                ).toFixed(2)
                : 0;
...
router.get('/:courseId/analytics', (req, res) => {
    ...
    db.query(
        `SELECT 
            ...
            total_marks,
            status,
            ROUND((earned_marks / total_marks) * 100, 2) as percentage
         FROM student_grades
         WHERE course_id = ? AND student_id = ?
         AND earned_marks IS NOT NULL
...
router.get('/:courseId/export/pdf', (req, res) => {
    ...
    db.query(
        `SELECT 
            ...
            ROUND((earned_marks / total_marks) * 100, 2) as percentage, 
            status,
            ROUND((SUM(earned_marks) OVER()) / (SUM(total_marks) OVER()) * 100, 2) as overall_average
         FROM student_grades
         WHERE course_id = ? AND student_id = ?`,
        [courseId, studentId],
        (err, results) => {
            ...
            doc.rect(40, headerY, 520, 20).fill('#6a4fcf');

            doc.fillColor('#ffffff')
                .fontSize(10)
                .text('Assessment', 45, headerY + 5)
                .text('Category', 180, headerY + 5)
                .text('Earned', 280, headerY + 5)
                .text('Total', 340, headerY + 5)
                .text('%', 400, headerY + 5)
                .text('Status', 450, headerY + 5);
            ...
            results.forEach((g, i) => {
                ...
                doc.fillColor('#000')
                    ...
                    .text(
                        g.earned_marks !== null && g.earned_marks !== undefined
                            ? g.earned_marks
                            : '-',
                        280,
                        y + 5
                    ...
                    .text(
                        g.total_marks !== null && g.total_marks !== undefined
                            ? g.total_marks
                            : '-',
                        340,
                        y + 5
